...
def compute_similarity(ifname,placefile,keyterm,date,Ts):
    ...
    optimize(tweet_cur_window,weight,ofname,Ts,0.4,0.6,new)
...
def optimize(tweet,weight,ofname,L,A1,A2,new):
    ...
    try:
        ...
        for v in m.getVars():
            if v.x==1:
                ...
                if len(temp)==2:
                    final[tweet_word[int(temp[1])][2]]=new[tweet_word[int(temp[1])][2]]
                    
    except GurobiError as e:
        print(e)
        sys.exit(0)
    sorted_x = sorted(final.items(), key=lambda kv: kv[1])
    #for x in sorted_x:
        #print(x)
    for x in sorted_x:
        fo.write(x[0])
