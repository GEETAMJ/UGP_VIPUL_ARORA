...
def process_tweet(ifname,placefile,ofname):
    ...
    command = Tagger_Path + './ark-tweet-nlp-0.3.2/runTagger.sh --output-format conll temp.txt > tag.txt'
    os.system(command)
    ...
    for l in fp:
        ...
        if len(wl)>1:
            ...
            for x in temp:
                if len(x) > 0:
                    try:
                        s = s + x + ' '#.encode(encoding="ascii",errors="ignore") + ' '
                    except Exception as e:
                        print("pass")
                        pass
            ...
            for x in All:
                if len(x) > 0:
                    try:
                        z = z + x + ' '#.encode(encoding="ascii",errors="ignore") + ' '
                    except Exception as e:
                        print(e)
                        pass
            temp1 = Tweet[index]
